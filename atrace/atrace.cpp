 ...
 */
#define __STDC_FORMAT_MACROS 1 // Needed on non-Android, before inttypes.h, for PRId64.
...
#else
        fprintf(stderr, "isCategorySupported: stub on non-android\n");
#endif
        return false;
    }
...
#else
    // fine to not warn on non-android.
#endif
    return true;
}
...
#else
    fprintf(stderr, "setTagsProperty: stub on non-android\n");
#endif
    return true;
}
...
#else
    if (strlen(cmdline) > 0) {
        fprintf(stderr, "setAppCmdlineProperty: stub on non-android\n");
    }
#endif
    return true;
}
...
#else
    // ### implement me
    fprintf(stderr, "verifyKernelTraceFuncs: stub on non-android\n");
#endif
    return true;
}
...
#else
    // ### implement me
    fprintf(stderr, "setCategoriesEnableFromFile: stub on non-android\n");
#endif
    return true;
}
